function RequestView({
  ...
  const myRequests = useMemo(() => {
    ...
    return (requests || [])
      .filter(r => r.employeeId === currentUser.colaboradorId && r.type !== 'Escala de Trabalho' && r.type !== 'Ajuste de Escala')
  ...
  return (
    <div className="dashboard-grid requests-page-container" style={{ gap: '24px' }}>
      <header className="topbar glass-card" style={{ marginBottom: '8px', padding: '24px', borderRadius: '16px', border: '1px solid var(--line)' }}>
        <div>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 800, letterSpacing: '-0.02em', color: 'var(--title)' }}>
            {editingRequestId ? 'Editar agendamento' : 'Novo agendamento'}
          </h2>
          <p style={{ color: 'var(--muted)', marginTop: '4px', fontWeight: 500 }}>
            Planeje ausências com inteligência de dados e visibilidade total do time.
          </p>
        </div>
        <div className="badge-row" style={{ display: 'flex', gap: '12px' }}>
          {editingRequestId && <span className="status-pill warning" style={{ cursor: 'pointer' }} onClick={cancelEdit}>Modo Edição (Clique para cancelar)</span>}
          <span className="dash-micro-badge glass">Visualização Unificada</span>
        </div>
      </header>

      <section className="form-grid-layout" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '24px' }}>
        <div className="form-panel">
          <div className={`card glass-card ${editingRequestId ? 'editing-pulse' : ''}`} style={{ padding: '28px', borderRadius: '24px', border: '1px solid var(--line)', boxShadow: 'var(--shadow)' }}>
            ...
            <div className="field-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
              ...
              <div className="field" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--title)' }}>Tipo de Ausência</label>
                <select value={form.type} onChange={(e) => updateField('type', e.target.value)} style={{ width: '100%' }}>
              ...
              <div className="field" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--title)' }}>Data Inicial</label>
                <input type="date" value={form.startDate} onChange={(e) => updateField('startDate', e.target.value)} style={{ width: '100%' }} />
              </div>
              <div className="field" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--title)' }}>Data Final</label>
                <input type="date" value={form.endDate} onChange={(e) => updateField('endDate', e.target.value)} style={{ width: '100%' }} />
              </div>
              <div className="field" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--title)' }}>Plano de Cobertura</label>
                <input value={form.coverage} onChange={(e) => updateField('coverage', e.target.value)} placeholder="Quem assume?" style={{ width: '100%' }} />
              </div>
              <div className="field" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--title)' }}>Gestor / Área</label>
                <input value={formEmployee?.manager || 'Não definido'} readOnly style={{ width: '100%', background: 'var(--panel-strong)', opacity: 0.8, cursor: 'not-allowed' }} />
              </div>
            </div>

            <div className="field" style={{ marginTop: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--title)' }}>Observações e Contexto</label>
              <textarea 
                value={form.note} 
            ...
            <div className="action-row" style={{ marginTop: '36px', display: 'flex', gap: '16px' }}>
              <button
                ...
                style={{ 
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  boxShadow: '0 8px 20px rgba(51, 204, 204, 0.3)',
        ...
        <div className="form-panel">
          <div className="card glass-card" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <section className="management-section" style={{ marginTop: '12px' }}>
         <div className="card glass-card" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
